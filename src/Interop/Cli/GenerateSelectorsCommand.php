...
class GenerateSelectorsCommand extends ContainerAwareCommand
{
    ...
    private function getSelectorClassContent(string $entityFileName, string $subPath)
    {
        ...
        if (! empty($subPath)) {
            ...
            $selectorUseStatament = PHP_EOL . 'use App\\Base\\Selectors\\AbstractSelector;' . PHP_EOL;

        }
        $selectorClassName = $this->getSelectorClassName($entityFileName);
        return <<<CONTENT
<?php
namespace App\Base\Selectors{$selectorSubNamespace};
{$selectorUseStatament}
class {$selectorClassName} extends AbstractSelector
{
...
CONTENT;
    ...
    {
        return preg_replace(['/^Abstract/', '/.php$/'], '', $entityFileName) . 'Selector';
    }
